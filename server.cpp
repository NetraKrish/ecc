...
rhs)
{
return (lhs.ec_ == rhs.ec_) && (lhs.x_ == rhs.x_) &&
(lhs.y_ == rhs.y_);
}
...
{
int col = 0;
typename EllipticCurve<P>::m_table_t::iterator iter =
m_table_.begin();
...
cout << "Encrypted message from Netra to Shaurya = (m.x*Pk.x, m.y*Pk.y) =" <<"(" << c1 << ", " << c2 << ")\n\n";
...
cout << "\nThukila's decrypted message from Netra is = (" << m1d << ", "<< m2d << ")" << endl;
